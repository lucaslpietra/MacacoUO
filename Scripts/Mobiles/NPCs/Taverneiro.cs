...
{
	public class Taverneiro : BaseVendor
	{
		...
		public Taverneiro(Serial serial)
			: base(serial)
		{ }
		...
		{
			var taming = from.Skills[SkillName.Begging].Value;
			var anlore = from.Skills[SkillName.Anatomy].Value;
			var vetern = from.Skills[SkillName.Healing].Value;
		...
		public void BeginStable(Mobile from)
		{
			...
			if ((from.Backpack == null || from.Backpack.GetAmount(typeof(Gold)) < 100) && Banker.GetBalance(from) < 100)
			{
			 ...
			 */
			from.SendLocalizedMessage("Que mercenario deseja colocar no hotel?");
		...
		public void EndStable(Mobile from, BaseCreature pet)
		{
			...
			if (!pet.Body.IsHuman)
			{
				SayTo(from, "Nao sei lidar com animais..."); // HA HA HA! Sorry, I am not an inn.
			}
			...
			else if ((from.Backpack != null && from.Backpack.ConsumeTotal(typeof(Gold), 100)) || Banker.Withdraw(from, 100))
			{
		...
		public override void OnSpeech(SpeechEventArgs e)
		{
			...
			else if (!e.Handled && e.HasKeyword(0x0009) || e.Speech.Contains("retirar")) // *claim*
			{
				e.Handled = true;
		...
		private class StableEntry : ContextMenuEntry
		{
			...
			private readonly Taverneiro m_Trainer;

			public StableEntry(Taverneiro trainer, Mobile from)
				: base(6126, 12)
			{
				m_Trainer = trainer;
				m_From = from;
		...
		private class ClaimListGump : Gump
		{
			...
			public ClaimListGump(Taverneiro trainer, Mobile from, List<BaseCreature> list)
				: base(50, 50)
			{
				m_Trainer = trainer;
				m_From = from;
				...
				AddHtml(
					...
					275,
					20,
                    "<BASEFONT COLOR=#000008>Selecione o mercenario para retirar:</BASEFONT>",
					false,
					false);
		...
		private class ClaimAllEntry : ContextMenuEntry
		{
			...
			public ClaimAllEntry(Taverneiro trainer, Mobile from)
				: base(6127, 12)
			{
				m_Trainer = trainer;
				m_From = from;
		...
		private class StableTarget : Target
		{
			...
			public StableTarget(Taverneiro trainer)
