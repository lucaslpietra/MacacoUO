...
namespace Server.Gumps
{
    public class BoardGameScoresGump : BoardGameGump
    {
        ...
        public BoardGameScoresGump(Mobile owner, BoardGameControlItem controlitem, int page) : base(owner, controlitem)
        {
            ...
            AddLabel(_X, _Y, 1152, "Nome");
            AddLabel(_X + 150, _Y, 1152, "Pontos");
            AddLabel(_X + 200, _Y, 1152, "Vitorias");
            AddLabel(_X + 250, _Y, 1152, "Derrotas");

            for (int i = listingstart; i < listingend; i++)
            {
                AddLabel(_X, _Y += 20, 1152, _PlayerScores[i].Player.Name);
                AddLabel(_X + 150, _Y, 1152, _PlayerScores[i].Score.ToString(CultureInfo.InvariantCulture));
                AddLabel(_X + 200, _Y, 1152, _PlayerScores[i].Wins.ToString(CultureInfo.InvariantCulture));
                AddLabel(_X + 250, _Y, 1152, _PlayerScores[i].Losses.ToString(CultureInfo.InvariantCulture));
            }
        ...
        protected void AddPageButtons()
        {
            ...
            AddLabel(Width - 120, _Y, 88, "Proxima");
