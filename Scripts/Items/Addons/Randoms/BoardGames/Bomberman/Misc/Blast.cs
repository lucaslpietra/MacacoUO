    ...
    public class BombBlast
    {
        ...
        public void Explode()
        {
            ...
            foreach (Item item in ie)
            {
                ...
                {
                    var obstacle = (BombermanObstacle) item;
            ...
            if (_Strength > 0)
            {
                var newblast = new BombBlast(_Location, _Map, _Direction, _BombBag, _Planter, _Strength - 1, _BaddaBoom);
            }
        }

        protected void RenderBlast()
        {
            Effects.SendLocationEffect(new Point3D(_Location.X + 1, _Location.Y + 1, _Location.Z), _Map,
                Utility.RandomList(0x36CB, 0x36BD, 0x36B0), 10);
        }
        ...
        protected class BlastTimer : Timer
        {
            ...
            public BlastTimer(BombBlast bombblast)
                : base(TimeSpan.FromMilliseconds(BombermanSettings.BLAST_DELAY), TimeSpan.FromSeconds(1.0))
            {
