...
namespace Server.Gumps
{
    public class SelectBombermanStyleGump : SelectStyleGump
    {
        ...
        public SelectBombermanStyleGump(Mobile owner, BoardGameControlItem controlitem) : base(owner, controlitem)
        {
            ...
            AddLabel(20, 14, 1152, "Estilos de Bomberman");
            ...
            {
                var index = (int) Enum.Parse(typeof(BombermanStyle), stylename);
            ...
            AddLabel(_X, _Y += 30, 1152, "Largura:");
            AddLabel(_X + 90, _Y, 1152, "Altura:");

            if (!_ControlItem.SettingsReady)
            {
                AddTextField(_X + 50, _Y, 30, 20, 1, _ControlItem.BoardWidth.ToString(CultureInfo.InvariantCulture));
                AddTextField(_X + 140, _Y, 30, 20, 2, _ControlItem.BoardHeight.ToString(CultureInfo.InvariantCulture));
            }
            else
            {
                AddLabel(_X + 50, _Y, 1152, _ControlItem.BoardWidth.ToString(CultureInfo.InvariantCulture));
                AddLabel(_X + 140, _Y, 1152, _ControlItem.BoardHeight.ToString(CultureInfo.InvariantCulture));
            }


            AddLabel(_X, _Y += 40, 1152, "Pronto para comecar:");
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (selection >= 0 && !_ControlItem.SettingsReady)
            {
                ...
                {
                    from.SendMessage("Valor Invalido");
                }
            }
