...
namespace VitaNex.Modules.AutoPvP
{
	public class PvPBattlesUI : TreeGump
	{
		private const string _Info = //
            "As batalhas PvP permitem que você entre em combate com outros jogadores em diferentes cenários." +
            "\n As batalhas nao perdem loot (a menos que especificado)!" +
            "\n Cada batalha pode oferecer uma recompensa para vencedores e perdedores!" +
            "\n Junte-se a uma batalha agora e ganhe sua classificação entre os melhores jogadores de {0}!";

        private static readonly string _States = //
			"A fila para batalhar esta aberta.".WrapUOHtmlColor(Color.RoyalBlue) +
			"\nPreparando: Permite preparar para a batalha.".WrapUOHtmlColor(Color.OrangeRed) +
			"\nBatalhando: A batalha esta acontecendo!".WrapUOHtmlColor(Color.MediumSeaGreen) +
			"\nFinal: A batalha terminou.".WrapUOHtmlColor(Color.IndianRed);
		...
		public static string GetNodePath(PvPBattle o)
		{
			...
			var fmtCat = "Batalhas|" + (useCat ? "{0}|{1}" : "{1}");
		...
		protected override void OnSelected(TreeGumpNode oldNode, TreeGumpNode newNode)
		{
			...
			if (oldNode.IsChildOf("Temporadas") && !newNode.IsChildOf("Temporadas"))
			{
				Season = null;
			}
			else if (oldNode.IsChildOf("Batalhas") && !newNode.IsChildOf("Batalhas"))
			{
				Battle = null;
			}
			else if (oldNode.IsChildOf("Perfil") && !newNode.IsChildOf("Perfil"))
			{
				Profile = null;
			}
		}
		...
		protected override void CompileNodes(Dictionary<TreeGumpNode, Action<Rectangle, int, TreeGumpNode>> list)
		{
			...
			if (_Seasons.Count > 0)
			{
				list["Temporadas"] = (b, i, n) => CompileSeasonsLayout(b);

				foreach (var o in _Seasons)
				{
					var season = o;

					list["Temporadas|Temporada " + season.Number] = (b, i, n) => CompileSeasonLayout(b, season);
				}
			}
			...
			if (_Battles.Count > 0)
			{
				list["Batalhas"] = (b, i, n) => CompileBattlesLayout(b, null);
				...
				foreach (var bo in _Battles)
				{
					if (useCat)
					{
						...
						list["Batalhas|" + cat] = (b, i, n) => CompileBattlesLayout(b, cat);
					}
			...
			list["Perfil"] = (b, i, n) => CompileTopRankedLayout(b);
			...
			if (user != null)
			{
				...
				if (pro != null)
				{
					list["Perfil|Meu Perfil"] = (b, i, n) => CompileProfileLayout(b, pro);
				}
			}
		...
		private int ResolveProfiles(Dictionary<PvPProfile, int> pool)
		{
			...
			foreach (var p in AutoPvP.GetSortedProfiles(order, profiles))
			{
				...
				if (p.Owner.AccessLevel > AccessLevel.VIP)
				{
					if (!edit)
					{
						continue;
			...
			int col)
		{
			...
			if (row < 0) // headers
			{
				switch (col)
				{
					...
					case 1:
					{
						AddHtml(x, y, w, h, "Nome", HtmlColor, Color.Empty);
						AddTooltip(3000547); // Player
					}
						break;
					case 2:
					{
						AddHtml(x, y, w, h, "Pontos", HtmlColor, Color.Empty);
					...
					case 3:
						...
						break;
					case 4:
						AddHtml(x, y, w, h, "Vitorias", HtmlColor, Color.Empty);
						break;
					case 5:
						AddHtml(x, y, w, h, "Derrotas", HtmlColor, Color.Empty);
						break;
				}
			}
			else
		...
		protected virtual void CompileSeasonLayout(Rectangle b, PvPSeason season)
		{
			...
			text = "Temporada " + (Numeral)season.Number;
			text = text.WrapUOHtmlBig();
			text = text.WrapUOHtmlCenter();
		...
		protected virtual void RenderBattlesTable(int x, int y, int w, int h, PvPBattle battle, int row, int col)
		{
			...
			switch (col)
			{
				case 0: // Name
				{
					...
					text = battle.Ranked ? "Rankiado" : "Nao Rankiado";
		...
		protected virtual void CompileBattleLayout(Rectangle b, PvPBattle battle)
		{
			...
			text = battle.Name + " (" + (battle.Ranked ? "Rankquiado" : "Nao Rankiado") + ")";
			text = text.WrapUOHtmlBig();
			text = text.WrapUOHtmlCenter();
			...
			if (!battle.Hidden || CanEdit)
			{
				...
				if (User is PlayerMobile)
				{
					...
					else
					{
						...
						{
							text = UniGlyph.CircleX + " Sair Fila";
						...
						{
							text = UniGlyph.StarFill + " Entrar Fila";
						...
						{
							text = UniGlyph.CircleDot + " Assistir";
						...
						if (!battle.IsInternal)
						{
							...
							if (pro != null)
							{
								...
								{
									text = UniGlyph.Coffee + " Desinscrever";
								...
								{
									text = UniGlyph.Coffee + " Inscricao";
			...
			if (!battle.RequireCapacity)
			{
				...
				AddHtml(b.X + 5, b.Y, b.Width - 10, 20, "Participantes", Color.White, Color.Empty);
				AddHtml(b.X + 5, b.Y, b.Width - 10, 20, text, Color.White, Color.Empty);
				AddTooltip(1018090); // Players
			...
			if (battle.SpectateAllowed || battle.Spectators.Count > 0)
			{
				...
				AddRectangle(b.X, b.Y, b.Width, 20, bgcol, true);
				AddHtml(b.X + 5, b.Y, b.Width - 10, 20, "Plateia", Color.White, Color.Empty);
				AddHtml(b.X + 5, b.Y, b.Width - 10, 20, text, Color.White, Color.Empty);
				AddTooltip(1018090); // Players
			...
			if (battle.QueueAllowed || battle.Queue.Count > 0)
			{
				...
				AddRectangle(b.X, b.Y, b.Width, 20, bgcol, true);
				AddHtml(b.X + 5, b.Y, b.Width - 10, 20, "Na Fila", Color.White, Color.Empty);
				AddHtml(b.X + 5, b.Y, b.Width - 10, 20, text, Color.White, Color.Empty);
				AddTooltip(1018090); // Players
			...
			AddRectangle(b.X, b.Y, b.Width, 20, bgcol, true);
			AddHtml(b.X + 5, b.Y, b.Width - 10, 20, "Times", Color.White, Color.Empty);
			AddHtml(b.X + 5, b.Y, b.Width - 10, 20, text, Color.White, Color.Empty);
				...
				case 3:
				{
					...
					if (user != null)
					{
						...
						if (!team.Battle.Queue.TryGetValue(user, out t))
						{
							if (team.IsFull)
							{
								text = UniGlyph.StarEmpty + " Cheio";
							}
							else
							{
								text = UniGlyph.StarFill + " Entrar";
		...
		protected virtual void CompileOverviewLayout(Rectangle b)
		{
			...
			{
				build.AppendLine("{0:#,0} batalhas.", stats.Battles - stats.BattlesInternal);
			}

			build.AppendLine("{0:#,0} batalhas em fila.", stats.BattlesQueueing);
			build.AppendLine("{0:#,0} batalhas preparando.", stats.BattlesPreparing);
			build.AppendLine("{0:#,0} batalhas acontecendo.", stats.BattlesRunning);
			build.AppendLine("{0:#,0} batalhas terminando.", stats.BattlesEnded);

			build.AppendLine("{0:#,0} jogadores em fila.", stats.Queueing);
			build.AppendLine("{0:#,0} jogadores batalhando.", stats.Participants);
			build.AppendLine("{0:#,0} jogadores assistindo.", stats.Spectators);
