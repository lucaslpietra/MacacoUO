			...
			if (Selected != null && !Selected.Deleted)
			{
				...
				if (user != null)
				{
					...
					if (Selected.Battle.IsParticipant(user, out team))
					{
						if (team == Selected)
						{
							list.AppendEntry(new ListGumpEntry("Sair", b => Selected.Battle.Quit(User, true)));
						}
					}
					else
					{
						if (Selected.Battle.IsQueued(user))
						{
							...
							{
								list.AppendEntry(new ListGumpEntry("Sair da Fila", b => Selected.Battle.Dequeue(user)));
							}
						}
						else if (!Selected.Battle.AutoAssign && Selected.Battle.CanQueue(user))
						{
							list.AppendEntry(new ListGumpEntry("Entrar na Fila", b => Selected.Battle.Enqueue(user, Selected)));
						}
					}
				}
			}
