	...
	public static partial class AutoPvP
	{
		...
		public static void SetDeserter(PlayerMobile pm, bool state, bool message)
		{
			...
			if (state && CMOptions.Advanced.Misc.DeserterLockout > TimeSpan.Zero)
			{
				...
				if (message)
				{
					pm.SendMessage(0x22, "Voce desertou seu time e precisa aguardar para entrar em outro campo de batalha.");
				}
				...
				foreach (var a in pm.Account.FindSharedAccounts().Where(a => a != pm.Account && !Deserters.ContainsKey(a)))
				{
					...
					if (p != null)
					{
						p.SendMessage(0x22, "{0} desertou da batalha!", pm.RawName);
						p.SendMessage(0x22, "Voce precisa aguardar para entrar novamente por usar contas compartilhadas.");
					}
				}
			}
			else
			{
				...
				{
					pm.SendMessage(0x55, "Voce nao e mais um desertor de batalhas.");
				}
				...
				Deserters.RemoveRange(
					o =>
					{
						...
						if (o.Value == t)
						{
							...
							if (p != null)
							{
								p.SendMessage(0x55, "Voce nao e mais um desertor de batalhas.");
							}
