...
namespace VitaNex.Modules.AutoPvP
{
	...
	public abstract partial class PvPBattle : IEquatable<PvPBattle>, IComparable<PvPBattle>
	{
		...
		{
			LocalBroadcast("Morte Subita ! Prepare-se !");
		}

		protected virtual void OnSuddenDeathEnd()
		{
			LocalBroadcast("Morte Subita terminou.");
		}
		...
		protected virtual void OnRewarded(PlayerMobile pm, IEntity reward)
		{
			...
			{
				pm.SendMessage("Voce foi recompensado por participar da {0}.", Name);
			}
		}
		...
		protected virtual void OnWin(PlayerMobile pm)
		{
			...
			WorldBroadcast("{0} ganhou {1}!", pm.Name, Name);
			...
			if (Schedule != null && Schedule.Enabled)
			{
				html.AppendLine(
					Schedule.NextGlobalTick != null
						? "Esta batalha tem um calendario.".WrapUOHtmlColor(Color.LawnGreen, col)
						: "Esta batalha eh rotineira mas nao esta no calendario.".WrapUOHtmlColor(Color.IndianRed, col));
			}
			else
			{
				html.AppendLine("Esta batalha eh automatica (nao tem calendario).".WrapUOHtmlColor(Color.LawnGreen, col));
			}
		...
		public virtual void GetHtmlCommandList(Mobile viewer, StringBuilder html, bool preview = false)
		{
			...
			html.AppendLine("Comandos".WrapUOHtmlBig());
			html.AppendLine("(Para usar na batalha ou na plateia)".WrapUOHtmlBig());
			html.AppendLine();
