...
namespace VitaNex.Modules.AutoPvP
{
	public abstract partial class PvPBattle
	{
		...
		{
			return Options.Rules.CanDamageOwnTeam && State == PvPBattleState.Batalhando;
		}

		public virtual bool CanDamageEnemyTeam(PlayerMobile damager, PlayerMobile target)
		{
			return Options.Rules.CanDamageEnemyTeam && State == PvPBattleState.Batalhando;
		}

		public virtual bool CanHealOwnTeam(PlayerMobile healer, PlayerMobile target)
		{
			return Options.Rules.CanHealOwnTeam && (State == PvPBattleState.Preparando || State == PvPBattleState.Batalhando);
		}

		public virtual bool CanHealEnemyTeam(PlayerMobile healer, PlayerMobile target)
		{
			return Options.Rules.CanHealEnemyTeam && (State == PvPBattleState.Preparando || State == PvPBattleState.Batalhando);
		}
		...
		{
			team.Broadcast("{0} entrou na batalha.", pm.RawName);
		...
		{
			team.Broadcast("{0} deixou a batalha.", pm.RawName);
