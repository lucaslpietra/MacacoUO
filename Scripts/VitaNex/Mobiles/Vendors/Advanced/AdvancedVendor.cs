	...
	public abstract class AdvancedVendor : BaseVendor, IAdvancedVendor
	{
		...
		public override bool OnBuyItems(Mobile buyer, List<BuyItemResponse> list)
		{
			...
			if (fullPurchase)
			{
				...
				{
					SayTo(buyer, true, "Pra staff eh de graca neh patrao.");
				}
				else if (fromBank)
				{
					SayTo(
						buyer,
						true,
						"Voce gastou {0:#,0} {1}, que foi retirado de sua conta.",
						totalCost,
			...
			else
			{
				if (buyer.AccessLevel >= AccessLevel.GameMaster)
				{
					SayTo(
						buyer,
						true,
						"Voce nao pode comprar nada aqui.");
				}
				else if (fromBank)
				{
					SayTo(
						buyer,
						true,
						"Voce pagou {0:#,0} {1}, infelizmente nao puder vender tudo que voce pediu..",
						totalCost,
						CashName.GetString(buyer));
				}
				else
