...
namespace Server.Items
{
    public class ExodusSummoningRite : BaseDecayingItem
    {
        ...
        {
            this.Name = "Ritual de Invocacao do Exodo";
            this.PartyLoot = true;
            this.Weight = 1;
            this.Hue = 1910;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendMessage("Selecione onde deseja usar este estranho pergaminho"); // Which Summoning Tome do you wish to use this on? 
                ...
                if (targeted is ExodusTomeAltar)
                {
                    ...
                    if (altar.CheckParty(altar.Owner, from))
                    {
                        ...
                        if (!RiteRitual)
                        {
                            ...
                            from.Say("Coloquei o rito no tomo..."); // You place the rite within the tome and begin to meditate...
                            altar.Rituals.Find(s => s.RitualMobile == from).Ritual1 = true;
                            m_Deed.Delete();
                            from.SendMessage(from.Name+" iniciou o ritual"); // ~1_PLAYER~ has read the Summoning Rite! 
                        }
                        else
                        {
