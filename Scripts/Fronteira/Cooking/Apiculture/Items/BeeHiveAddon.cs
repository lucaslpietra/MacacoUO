...
{	
	public class apiBeeHive : BaseAddon
	{
        ...
        public static readonly int MaxHoney = Config.Get("BeeHive.MaxWax", 20);

        //public static readonly int MaxWax = 10;    //maximum amount of wax
        public static readonly int MaxWax = Config.Get("BeeHive.MaxWax", 10);

        //public static readonly bool LessWax = true; //wax production is slower then honey (realistic)
        public static readonly bool LessWax = Config.Get("BeeHive.LessWax", true);
		...
		public HiveHealth OverallHealth
		{
			get
			{
				...
				if ( perc < 33 )
					return HiveHealth.Morrendo;
				else if ( perc < 66 )
					return HiveHealth.Doente;
				else if ( perc < 100 )
					return HiveHealth.Saudavel;
				else
					return HiveHealth.Prosperando;
			}
		}
		...
		{
			get { return m_Status != HiveStatus.Vazio; }
		}
		...
		public ResourceStatus ScaleWater()
		{
			...
			if ( perc < 33 )
				return ResourceStatus.MuitoBaixo;
			else if ( perc < 66 )
				return ResourceStatus.Baixo;
			else if ( perc < 101 )
				return ResourceStatus.Normal;
			else if ( perc < 133 )
				return ResourceStatus.Alto;
			else
				return ResourceStatus.MuitoAlto;
		...
		public void Pour( Mobile from, Item item )
		{
			...
			{
				LabelTo( from,"Voce nao pode jogar pocoes aqui.");
				return;
			}
			...
			{
				LabelTo( from, "Voce nao pode fazer isto!");
			}
		}
		...
		public bool ApplyPotion( PotionEffect effect, bool testOnly, out string message )
		{
			...
			else if (effect == PotionEffect.AgilidadeMaior )
			{
			...
			{
				message = "Esta pocao nao eh boa suficiente para usar aqui!";
				return false;
			}
			else
			{
				message = "Voce nao pode usar isto na colmeia!";
				return false;
			}

			if ( full )
			{
				message = "A colmeia esta enxarcada ja!";
				return false;
			}
			else
			{
				message = "Voce coloca um pouco da pocao na colmeia.";
				return true;
			}
		}
		...
		public void Grow()
		{
			if ( OverallHealth < HiveHealth.Saudavel )
			...
			else if ( ScaleFlower() < ResourceStatus.Baixo || ScaleWater() < ResourceStatus.Baixo )
			...
			else
			{
				...
				if( Wax < MaxWax )
				{
					...
					if( this.OverallHealth == HiveHealth.Prosperando )
						baseWax++;
			...
			if( HiveStage >= HiveStatus.Produzindo && !m_Bees.Visible )
				m_Bees.Visible = true;
		}
		...
		public bool ApplyMaladiesEffects()
		{
			...
			if( ScaleWater() < ResourceStatus.Baixo  )
				damage += ( 2 - (int)ScaleWater() ) * Utility.RandomMinMax( 3, 6 );

			if( ScaleFlower() < ResourceStatus.Baixo )
		...
		{
			//handle death
			if( HiveStage >= HiveStatus.Produzindo )
			{
				Population--;
	...
	public class apiBeeHiveComponent : AddonComponent
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			if( m_Hive == null )
			{
				LabelTo( from, "Esta colmeia esta invalida. Use um machado nela." );
				return;
			}

			if( m_Hive.HiveStage == HiveStatus.Vazio )
			{
				LabelTo( from, "Esta colmeia esta vazia. Use um machado nela.");
				return;
			}
		...
		public override void AddNameProperty( ObjectPropertyList list )
		{
			...
			if( m_Hive.HiveStage == HiveStatus.Vazio )
				...
				list.Add( m_Hive.OverallHealth.ToString()+" Colmeia" );
		}
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if( m_Hive.HiveStage >= HiveStatus.Produzindo )
				list.Add( 1049644 , "Produzingo" );
			else if( m_Hive.HiveStage >= HiveStatus.Reproduzindo )
				list.Add( 1049644 , "Reproduzingo" );
			else if( m_Hive.HiveStage >= HiveStatus.Colonizando )
				list.Add( 1049644 , "Colonizando" );
			else
				list.Add( 1049644 , "Vazio" );

			if( m_Hive.HiveStage != HiveStatus.Vazio )
				list.Add( 1060663,"{0}\t{1}" ,"Idade", m_Hive.HiveAge + (m_Hive.HiveAge==1 ? " day" : " days") );
			if( m_Hive.HiveStage >= HiveStatus.Produzindo )
				list.Add( 1060662,"{0}\t{1}" ,"Colonia", m_Hive.Population + "0k bees" );
		}
