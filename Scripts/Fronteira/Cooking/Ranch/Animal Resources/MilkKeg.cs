...
/*	public enum TypeOfMilk
	...
	public class MilkKeg : Item
	{
		...
		{
			if (m_Quantity > 0) Name = "barril de leite " + GetAgeDescription() + " [" + m_MilkType + "]";
			else Name = "barril de leite";
		}
		
		[Constructable]
		public MilkKeg() : base( 6870 )
		{
			Name = "barril de leite";
			Weight = 10;
			Hue = 2500;
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			//{
				from.SendMessage("Selecione um container.");
			...
			protected override void OnTarget( Mobile from, object targ )
			{
				...
				else if (targ is MilkingBucket)
				{
					...
					if(bev.Quantity < 10)
					{
						if (bev.MilkType == t_mk.MilkType || bev.Quantity < 1)
						{
							...
							bev.UpdateName();
							from.PlaySound( 0x4E );
							from.SendMessage("Voce colocou o leite no keg.");
						}
						else
						{
							...
							bev.UpdateName();
							from.PlaySound( 0x4E );
							from.SendMessage("Voce colocou o leite no barril.");
						}
					}
					else from.SendMessage("Isto esta cheio!");
				}
				...
				else if (targ is MeasuringCup)
				{
					if (t_mk.Quantity > 0)
					{
						...
						from.SendMessage("Voce mede um copo de leite.");
						t_mk.Quantity -= 1;
						t_mk.InvalidateProperties();
						t_mk.UpdateName();
					}
					else from.SendMessage("O barril esta vaziu!");
				}
				else
				{
