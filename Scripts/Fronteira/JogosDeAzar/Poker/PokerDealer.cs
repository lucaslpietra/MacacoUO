...
namespace Server.Engines.TexasHoldem
{
	public class PokerDealer : Mobile
	{
		...
		public bool Active
		{
			...
			set
			{
				...
				foreach (PokerPlayer p in toRemove)
				{
					p.Owner.SendMessage(
						0x22,
						"O jogo foi inativado e voce recebeu seu dinheiro de volta.");
					Game.RemovePlayer(p);
				}
		...
		public override void OnDoubleClick(Mobile from)
		{
            ...
            Shard.Debug("Poker: Mesa " + Game.Players.Count + " players ");

			if (!_MActive)
			{
				from.SendMessage(0x9A, "Esta mesa nao esta ativa");
			}
			else if (!InRange(from.Location, 8))
			{
				from.PrivateOverheadMessage(MessageType.Regular, 0x22, true, "Estou muito longe", from.NetState);
			}
			else if (MinBuyIn == 0 || MaxBuyIn == 0)
			{
				from.SendMessage(0x9A, "Esta mesa nao esta configurada");
			}
			else if (MinBuyIn > MaxBuyIn)
			{
				from.SendMessage(0x9A, "Esta mesa esta configurada errada");
			}
			else if (p != null && p.PokerGame != null && Game != p.PokerGame)
			{
				from.SendMessage(0x9A, "Voce nao pode entrar neste jogo.");
			}
            else if (Seats.Count < _MMaxPlayers)
            {
                from.SendMessage(0x9A, "Esta mesa esta com cadeiras faltando");
            }
			...
			{
				from.SendMessage(0x22, "Mesa cheia");
        ...
        public static void GetDealerPSeat(Mobile user)
        {
            ...
            {
                user.Target = new InternalTarget();
            }
        }
        ...
        public static void FoundDealer(Mobile User, Mobile target)
	    {
            if (target is PokerDealer)
            {
                ...
                {
                    User.SendMessage(0x22, "Nova cadeira criada.");
                }
            }
	    }
