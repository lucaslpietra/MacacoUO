...
namespace Server.Items
{
    public class TurboSlot : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendMessage("Voce precisa ser um membro para jogar essa maquina.");
                return;
            }
            ...
            else
            {
                if (m_IdleTimer < m_TimeOut)
                {
                    ...
                    if (from != m_InUseBy)
                        from.SendMessage("{0} ficou AFK, voce pode jogar agora.", m_InUseBy.Name);
                    InUseBy = from;
                }
            }
            ...
            {
                string text = String.Format("{0} esta jogando nessa maquina.", m_InUseBy.Name);
        ...
        {
            String[] RString = { "Desconhecido", null };
            ...
            {
                string[] sreturn = { "Gratiz", "Rodar!", null };
                payout = m_Cost;
                return sreturn;
            }
        ...
        private string[] GetRewardStr(string[] RString, int index, int sindex, out decimal rewardamount)
        {
            ...
            switch (m_SlotTheme)
            {
                case SlotThemeType.GruesomeGambling:
                    ...
                    {
                        case 0:
                            RString[sindex] = "Esqueleto Acorrentado";
                            if (sindex == 0)
                                RString[1] = "";
                            rewardamount = 500000;
                            return RString;
...
#endif
            from.PlaySound(739);
            if (from.Hidden && from.AccessLevel <= AccessLevel.VIP) // Don't let someone sit at the slots and play hidden
            {
                from.Hidden = false;
                from.SendMessage("Voce se revelou para jogar!");
            }
            m_FreeSpin = false;
            if ((m_ProgressiveMaster != null && !m_ProgressiveMaster.Deleted && ((TurboSlot)m_ProgressiveMaster).ProgIsMaster) || m_isProgMaster)
            {
            ...
            if (m_ReelOne == m_ReelTwo && m_ReelOne == m_ReelThree) // Jackpot!
            {
                if (r1index == m_Symbols[8] && m_BonusRound != BonusRoundType.None && !m_Profile)
                {
                    switch (m_BonusRound)
                    {
                        case BonusRoundType.MinerMadness:
                            {
                                ...
                                from.SendMessage(38, "Voce ganhou um round bonus.");
                                from.PlaySound(1035);
                                ...
                                if (m_Won > m_CreditCashOut && m_InUseBy.AccessLevel <= AccessLevel.VIP)
                                    DoCashOut(from);
                                return;
                            }
                        ...
                        default:
                            {
                                from.SendMessage(38, "Maquina precisa de manutencao.");
                                SlotOffline(8004);
                                return;
                            }
                    }
                        ...
                        case 0:
                            {
                                ...
                                {
                                    string text = String.Format("{0} ganhou {1} moedas!!!", from.Name, payout);
                                    AnnounceJackpot(from, text);
                                }
                                DoSpinWin(from, r1index, p, m_InUseBy.Female ? m_FemaleSounds[r1index] : m_MaleSounds[r1index], m_Sounds[r1index], m_JackpotText[r1index], m_JackpotEffect[r1index]);
                                ActivateBlinkTimer(0);
                        ...
                        default:
                            {
                                if (!m_Profile)
                                {
                                    from.SendMessage("Voce nao ganhou, tente novamente!");
                                    ...
                                    if (m_SlotTheme == SlotThemeType.GruesomeGambling)
                                    {
                                        ...
                                        {
                                            from.SendMessage("Voce deixou o demonio dentro dessa maquina irado!");
                                            BeginBleed(from);
                                        }
                                    }
                                    else if (m_SlotTheme == SlotThemeType.StatScrolls)
                                    {
                                        ...
                                        {
                                            from.SendMessage("Tentaculos atacam!");
                                            DoDrain(from);
                                        }
                                    }
        ...
        private void DoSpinWin(Mobile from, int r1index)
        {
            ...
            else
            {
                ...
                {
                    if (!m_Profile)
                        from.SendMessage(38, "Rodada Gratiz!");
                ...
                {
                    if (!m_Profile)
                        from.SendMessage("Voce perdeu, tente novamente !");
                    return;
                }
                if (!m_Profile)
                    from.SendMessage(38, "Multiplicador de payout invalido. Maquina precisa de manutencao.");
                SlotOffline(8005);
            }
        }
        ...
        private void DoSpinWin(Mobile from, int r1index, int winnings)
        {
            ...
            if (m_Profile || winnings == 0)
                return;
            from.SendMessage(38, "Voce ganhou {0} moedas!", winnings);
        }
                ...
                case SlotThemeType.GruesomeGambling:
                    {
                        switch (jackpotindex)
                        {
                            case 0:             //Hooded Shroud of Shadows
                                {
                                    Type t = typeof(HangingSkeletonDeed);
        ...
        public void DoCashOut(Mobile from)
        {
            ...
            {
                from.SendMessage("Voce nao esta mais jogando!");
                return;
            }
            ...
            if (m_Won < 1000)
            {
                try
                {
                    ...
                    from.SendMessage("{0} moedas foram adicionadas a sua mochila.", credit);
                }
                catch
                {
            ...
            else
            {
                try
                {
                    ...
                    from.SendMessage("Um cheque de {0} moedas foi adicionado a sua mochila.", credit);
                }
                catch
                {
                    from.SendMessage("There's a problem returning your gold, this slot machine is offline. Page for help.");
            ...
            if (credit >= 10000)
            {
                string text = String.Format("{0} esta retirando {1} moedas!", m_InUseBy.Name, credit);
                this.PublicOverheadMessage(0, this.Hue, false, text);
            }
        ...
        private void SetupOddsTable(PaybackType p, bool force)
        {
            ...
            if (500 > Utility.Random(2000) && m_CurrentPaybackType != PaybackType.Loose)
                m_CurrentPaybackType--;
            if (m_PaybackType == PaybackType.Random && m_InUseBy != null)
                ActivateRandomTimer();
		...
		{
			if (m_InUseBy == null || (m_InUseBy != null && m_InUseBy.AccessLevel <= AccessLevel.VIP))
			{
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            if (!m_Active)
            {
                if (m_ErrorCode == 0)
                    ...
                    list.Add(1060658, "Status\tQuebrada({0})", m_ErrorCode);
                return;
            }
            else if (m_InUseBy == null)
            {
                if (m_CardClubOnly)
                    list.Add(1060658, "Status\tDisponivel (Apenas Membros)");
                else
            ...
            {
                list.Add(1060658, "Status\tEm uso");
                list.Add(1060659, "Jogador\t{0}", m_InUseBy.Name);
                numberpos = 1060660;
            }
            if (m_LastWonBy != null)
            {
                list.Add(numberpos, "Ultimo Ganhador\t{0}", m_LastWonBy.Name);
                numberpos++;
                list.Add(numberpos, "Data\t{0}", m_LastWonByDate);
                numberpos++;
                list.Add(numberpos, "Quantidade\t{0}", m_LastWonAmount);
            }
        }
