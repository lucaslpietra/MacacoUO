    ...
    public class TurboSlotGump : Gump
    {
            ...
            : base(25, 25)
        {
            ...
            if (m_Slot.ShowPayback || (m_Slot.InUseBy != null && m_Slot.InUseBy.AccessLevel != AccessLevel.Player))
            {
                ...
                AddLabel(205, 65, headingcolor1, @"Payback Pct:");
                ...
                if (m_Slot.InUseBy != null && m_Slot.InUseBy.AccessLevel != AccessLevel.Player)
                {
                    ...
                    AddLabel(232, 80, headingcolor1, @"Probabilidade:");
            ...
            {
                AddLabel(230, lastjackpotstarty, headingcolor2, "Ultimo Ganhador:");
                ...
                AddLabel(205, lastjackpotstarty + 30, headingcolor2, "Data:");
                AddLabel(237, lastjackpotstarty + 30, paycolor, text);
                text = String.Format("{0:##,###,### Gold!}", m_Slot.LastWonAmount);
                AddLabel(202, lastjackpotstarty + 45, headingcolor2, "Valor:");
                AddLabel(237, lastjackpotstarty + 45, paycolor, text);
            }
            if (progressive)
            {
                Jackpot = m_Slot.GetJackpotPayoutStr(0, out payout);
                AddHtml(245, progstarty, 356, 23, @"<BASEFONT COLOR=#FFFFFF><U>Ganho Progressivo</U></BASEFONT>", (bool)false, (bool)false);
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            {
                from.SendMessage("Alguem jogou enquanto voce nao jogava. Clique novamente para jogar.");
                CloseAllGumps(from);
                return;
            }
            if (from.Serial != m_Slot.InUseBy.Serial)
            {
                from.SendMessage("Voce demorou demais e {0} pegou sua maquina!", m_Slot.InUseBy.Name);
                CloseAllGumps(from);
                return;
            }
            if (!m_Slot.Active || m_Slot.Deleted)
            {
                from.SendMessage("Maquina quebrada, chame um engenheiro.");
                if (m_Slot.Won != 0)
                {
                    from.SendMessage("Pegando {0} moedas.", m_Slot.Won);
            ...
            if (!from.Alive)
            {
                from.SendMessage("Morto nao joga cassino, ta chapado ?");
                if (m_Slot.Won != 0)
                {
                    from.SendMessage("Pegando {0} moedas de ouro.", m_Slot.Won);
                    m_Slot.DoCashOut(from);
                }
                CloseAllGumps(from);
                m_Slot.InUseBy = null;
            ...
            if (info.ButtonID == 0) // Close
            {
                if (m_Slot.Won != 0)
                {
                    from.SendMessage("Voce parou de jogar e pegou seu dinheiro.", m_Slot.Won);
                    m_Slot.DoCashOut(from);
                    CloseAllGumps(from);
                }
                m_Slot.InUseBy = null;
            ...
            if (info.ButtonID == 12) // Spin
            {
                ...
                else
                {
                    ...
                    */
                    if (from.Backpack.ConsumeTotal(typeof(Gold), m_Slot.Cost))
                    {
                        m_Slot.DoSpin(from);
                        m_Slot.LastPay = 0;
                    ...
                    {
                        from.SendMessage("Criando cheque de {0} moedas em sua mochila, voce pode rodar novamente.", amount);
                        from.SendGump(new TurboSlotGump(m_Slot, m_Symbols));
                    }
                    else
                    {
                        from.SendMessage("Voce precisa ter {0} moedas, ou creditos na maquina para jogar.", m_Slot.Cost);
                        from.SendGump(new TurboSlotGump(m_Slot, m_Symbols));
                    }
                }
                return;
            ...
            if (info.ButtonID == 14) // Withdraw m_Slot.CreditATMIncrements from bank
            {
                ...
                {
                    from.SendMessage("Esta maquina nao cabe mais creditos.");
                }
                else
                {
                    ...
                    if (from.BankBox.ConsumeTotal(typeof(Gold), amount))
                    {
                        ...
                        from.SendMessage("{0} moedas foram sacadas do seu banco e adicionadas a maquina.", amount);
                        Effects.PlaySound(new Point3D(m_Slot.X, m_Slot.Y, m_Slot.Z), m_Slot.Map, 501);
                    }
                    else
                        from.SendMessage("Voce nao tem dinheiro suficiente no banco.");
                }
                from.SendGump(new TurboSlotGump(m_Slot, m_Symbols));
                return;
            }
            from.SendMessage("Seu macro tentou algo estranho, chame um engenheiro !");
        }
    }
}
