...
{
    public class AcampamentosGump : Gump
    {
        private List<CampSiteAddon> m_List;


        public static void Initialize()
        {
            CommandSystem.Register("camps", AccessLevel.Administrator, new CommandEventHandler(_OnCommand));
        }
        ...
        {
            if (e.Mobile.HasGump(typeof(AcampamentosGump)))
                e.Mobile.CloseGump(typeof(AcampamentosGump));
            e.Mobile.SendGump(new AcampamentosGump(e.Mobile));
        }
        ...
        public int GetCountForIndex(int index)
        {
            ...
            List<CampSiteAddon> list = m_List;
        ...
        public AcampamentosGump(Mobile from)
            : this(from, 0, null)
        {
        }
        ...
        public AcampamentosGump(Mobile from, int page, List<CampSiteAddon> list, string filtro = null)
            : base(12, 24)
        {
            ...
            if (list == null)
                list = Acampamento.Points.Values.ToList();
            m_List = list;
            ...
            AddHtml(266, 32, 200, 32, "Acampamentos", LabelColor, false, false); // Recipe Book
            ...
            if (GetIndexForPage(page + 1) < m_List.Count)
            {
            ...
            for (int i = index; i < (index + count) && i >= 0 && i < m_List.Count; ++i)
            {
                ...
                //var r = 
                //AddHtml(316, y, 100, 20, element.GetRegion() != null && element.Region.Name != null ? element.Region.Name : "Desconhecido", 0xFFFFFF, false, false); // ~1_val~
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch (index)
            {
                case 1:
                    {
                        from.SendGump(new AcampamentosGump(from, m_Page, m_List, novoFiltro));
                        break;
                    }
                case 2:
                    {
                        if (m_Page > 0)
                            from.SendGump(new AcampamentosGump(from, m_Page - 1, m_List, novoFiltro));
