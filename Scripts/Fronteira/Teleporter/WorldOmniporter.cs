...
namespace Server.Items
{
	...
	public class WorldOmniporterGump : Gump
	{
		...
		
		{
			...
			if ( m_Staff ) AddHtml(363, 40, 50, 15, "<basefont color=#FFFFFF>Para</basefont>", false, false );					// staff see Omniporter usage
			if ( m_Staff ) AddHtml(396, 40, 50, 15, "<basefont color=#FFFFFF>De</basefont>", false, false );					// staff see Omniporter usage 
			AddHtml(75, 40, 150, 15, "<basefont color=#FFFFFF>Escolha:</basefont>", false, false );
			AddHtml(165, 12, 200, 15, "<basefont color=#FFFFFF>Portal Ethereo</basefont>", false, false ); 
			...
			if ( WO.Trammel && m_Reds )
			{
				...
				AddPageButton( "Trammel Cidades", Map.Trammel, p++, 1 );
			}
			...
			if ( WO.Felucca && WO.CanUse( m, false, true ) )
			{
				...
				AddPageButton("Felucca Cidades", Map.Felucca, p++, 3 );
			}
			...
			if ( WO.PublicMoongates && (WO.Felucca || (WO.Trammel && m_Reds)) )
			{
				...
				AddPageButton( "<basefont color=#FFFFFF>Gates Publicos</basefont>", null, p++, 5 );
			}
			...
			if ( WO.IlshenarShrines && m_Reds && m_HasLBR )
			{
				...
				AddPageButton( "Ilshenar Altares", Map.Ilshenar, p++, 7 );
			}
			...
			{
				GenerateMapListing( 11 );
				AddPageButton( "<basefont color=#FFFFFF>Outros</basefont>", null, p++, 11 );
			}
		...
		private void GenerateMapListing( int page )
		{
			...
			bool gates = name == "Gates Publicos";
		...
		{
			"Trammel", "Trammel Dungeons", "Felucca", "Felucca Dungeons",
			"Gates Publicos", "Ilshenar", "Ilshenar Altares", "Malas",
			"Tokuno", "TerMur", "Outros"
		};
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			string msg = "Voce se aproxima do portal ethereo, voce sente uma forte energia emanando do portal.";
		...
		public void PaymentConfirm_Callback( Mobile from, bool okay )
		{
			if ( okay )
			{
				...
				from.SendMessage("Voce prefere ficar.");
		}
