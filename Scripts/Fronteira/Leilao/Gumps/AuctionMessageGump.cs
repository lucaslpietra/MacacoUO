...
namespace Server.Leilaum
{
	...
	public class AuctionMessageGump : Gump
	{
		...
		private void MakeGump()
		{
			...
			if ( auction != null )
			{
				AddLabel(125, 85, LUtils.kRedHue, auction.ItemName );

				AddButton(65, 112, 9762, 9763, 0, GumpButtonType.Reply, 0);
				AddLabel(85, 110, LUtils.kLabelHue, AuctionSystem.ST[ 27 ] );
			}				
			else
			{
				AddLabel( 125, 85, LUtils.kRedHue, AuctionSystem.ST[ 28 ] );
			}
			
			AddHtml( 75, 170, 413, 120, m_HtmlMessage, (bool)true, (bool)false);
			AddLabel(75, 150, LUtils.kLabelHue, AuctionSystem.ST[ 29 ] );
			...
			if ( m_InformationMode )
			{
				...
				AddLabel( 125, 355, LUtils.kLabelHue, m_OkText );
			}
			else
			{
				AddButton(45, 300, 1147, 1149, 1, GumpButtonType.Reply, 0);
				AddLabel(125, 310, LUtils.kLabelHue, m_OkText);
				AddButton(45, 345, 1144, 1146, 2, GumpButtonType.Reply, 0);
				AddLabel(125, 355, LUtils.kLabelHue, m_CancelText);
			}

			if ( m_ShowExpiration && auction != null )
			{
				AddLabel( 55, 405, LUtils.kRedHue, 
