...
namespace Server.Mobiles
{
	...
	public class AngryFire : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
            ...
            defender.SendLocalizedMessage("A criatura te atinge com um fogo nervoso"); // The creature hits you with its Angry Fire.    
		}
	}
    ...
    public class FlurryForce : SpecialAbility
    {
        ...
        private class ExpireTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                m_Mobile.SendLocalizedMessage("Sua resistencia voltou ao normal"); // Your resistance to energy attacks has returned.
                DoExpire();
            }
        }
    }
    ...
    public class HowlOfCacophony : SpecialAbility
    {
        ...
        public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
        {
            ...
            defender.SendLocalizedMessage("Um som doi seus timpanos e voce perde seu balanco, nao consegue se mover"); // // A cacophonic sound lambastes you, suppressing your ability to move.
    ...
    public class GraspingClaw : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
            ...
            {
                timer.DoExpire();
                defender.SendLocalizedMessage("A criatura acerta outro golpe no seu estado fraco"); // The creature lands another blow in your weakened state.
            }
            else
                defender.SendLocalizedMessage("O golpe da criatura lhe fez sucetivel a ataques fisicos"); // The blow from the creature's claws has made you more susceptible to physical attacks.
            ...
            defender.AddResistanceMod(mod);
            defender.VirtualArmorMod += effect;
	...
	public class Inferno : SpecialAbility
	{
        ...
        public class ExpireTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                m_Mobile.SendLocalizedMessage("Suas resistencias voltam ao normal"); // Your resistance to fire attacks has returned.
                DoExpire();
            }
        }
	}
	...
	public class ManaDrain : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
            ...
            foreach (Mobile m in list)
            {
                ...
                m.SendMessage("Voce sente seu mana esvaziar!");
	...
	public class RagingBreath : SpecialAbility
	{
        ...
        public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
        {
            ...
            {
                timer.Stop();
                defender.SendLocalizedMessage("O Bafo de fogo da criatura continua te queimando"); // The creature's breath continues to burn you!
            }
            else
            {
                defender.SendLocalizedMessage("O bafo de fogo da criatura te queima"); // The creature's breath is burning you!
            }
        ...
        public static void EndFire(Mobile m)
        {
            if (_Table != null && _Table.ContainsKey(m))
            {
                _Table[m].Stop();
                _Table.Remove(m);
                m.SendLocalizedMessage("O bafo de fogo se dissipa"); // The fiery breath dissipates.
            }
        }
	...
	public class Repel : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
            defender.SendLocalizedMessage("A criatura reflete o ataque a voce"); // The creature repels the attack back at you.
            defender.FixedEffect(0x37B9, 10, 5);
	...
	public class SearingWounds : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
            ...
            defender.SendLocalizedMessage("O ataque causa uma ferida no impacto"); // The searing attack cauterizes the wound on impact.
		}
	...
	public class VenomousBite : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
			...
			foreach(var m in list)
			{
                ...
                m.SendLocalizedMessage(creature.Name+" te envenenou"); //  : poisoned you!
	...
	public class RuneCorruption : SpecialAbility
	{
		...
		public class ExpireTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                m_Mobile.SendMessage("A corrupcao de sua armadura se foi");
                DoExpire();
            }
        }
	}
    ...
    public class Anemia : SpecialAbility
    {
        ...
        public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
        {
            ...
            if (_Table.ContainsKey(defender))
            {
                defender.PublicOverheadMessage(MessageType.Regular, 0x25, true, "* yuck sangue podre *"); // * The creature is repulsed by your diseased blood. *
            }
            else
            {
                defender.OverheadMessage("* drenado *"); // *The creature drains some of your blood to replenish its health.*
    ...
    public class BloodDisease : SpecialAbility
    {
        ...
        public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
        {
            ...
            if (!_Table.ContainsKey(defender) && creature.InRange(defender, 1) && 0.25 > Utility.RandomDouble() && !FountainOfFortune.UnderProtection(defender))
            {
                ...
                defender.SendLocalizedMessage("Voce foi infectado com uma infeccao no sangue");
	...
	public class TailSwipe : SpecialAbility
	{
		...
		public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
		{
			...
			{
				defender.SendLocalizedMessage("A criatura te atordoou"); // You're stunned as the creature's tail knocks the wind out of you.
			...
			{
				defender.SendLocalizedMessage("O rabo da criatura bateu em voce com forca e voce ficou atordoado"); // You're left confused as the creature's tail catches you right in the face!
    ...
    public class Rage : SpecialAbility
    {
        ...
        public override void DoEffects(BaseCreature creature, Mobile defender, ref int damage)
        {
            ...
            {
                timer.DoExpire();
                defender.SendLocalizedMessage("A criatura continua em furia"); // The creature continues to rage!
            }
            else
                defender.SendLocalizedMessage("A criatura entra em furia, aumentando seu dano"); // The creature goes into a rage, inflicting heavy damage!
        ...
        private class ExpireTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                {
                    DoExpire();
                    m_Mobile.SendLocalizedMessage("A criatura fica em paz"); // The creature's rage subsides.
                }
            }
        }
    }
