...
namespace Server.Savings
{
	...
	public class SavingsAccount : Item
	{
		...
		private static void World_Save( WorldSaveEventArgs args )
		{
			...
			if ( AuctionConfig.InterestHour <= now.Hour && SavingsAccount.DateLastPaid != now.Date )
			{
				...
				foreach ( Item item in accounts )
				{
					...
					if ( Server.Savings.SavingsAccount.EnableTokens )
					...
					if ( null != box && null != box.Owner && box.Owner.Map != Map.Internal )
						box.Owner.SendMessage( global::Leilaum.Utilities.LUtils.kRedHue, "Interest has been paid on the {0} in your savings account!", AuctionConfig.EnableTokens ? "gold and tokens" : "gold" );
				}
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			...
			if ( !success )
			{
				string text = Server.Savings.SavingsAccount.EnableTokens ? ", tokens " : " ";
		...
		private static void ScaleSavingsAccounts( Mobile from, SavingsOption option, double percent )
		{
			...
			{
				Server.Savings.SavingsAccount sa = i as Server.Savings.SavingsAccount;
