...
namespace Server.Misc
{
    ...
    public class ControleRazor
    {
        ...
        public const string WarningMessage = "Nao consegui conectar com o Razor...";
    ...
    public class RazorFeatureEnforcer
    {
        ...
        private static void EventSink_Login(LoginEventArgs e)
        {
            ...
            if (m != null && m.NetState != null && m.NetState.Running)
            {
                ...
                m_Table[m] = t = Timer.DelayCall(ControleRazor.HandshakeTimeout, OnHandshakeTimeout_Callback, m);
                t.Start();
            }
        }
        ...
        private static void OnHandshakeTimeout(object state)
        {
            ...
            if (!ControleRazor.KickOnFailure)
            {
                Console.WriteLine("Player '{0}' failed to negotiate Razor features.", m);
            }
            else if (m.NetState != null && m.NetState.Running)
            {
                m.SendGump(new Gumps.WarningGump(1060635, 30720, ControleRazor.WarningMessage, 0xFFC000, 420, 250, null, null));

                if (m.AccessLevel <= AccessLevel.Player)
                {
                    m_Table[m] = t = Timer.DelayCall(ControleRazor.DisconnectDelay, OnForceDisconnect_Callback, m);
                    t.Start();
                }
            }
        }
        ...
        private sealed class BeginRazorHandshake : ProtocolExtension
        {
            ...
            {
                m_Stream.Write((uint)((ulong)ControleRazor.DisallowedFeatures >> 32));
                m_Stream.Write((uint)((ulong)ControleRazor.DisallowedFeatures & 0xFFFFFFFF));
            }
        }
    }
   */
