...
{
    public class CurrentRecipesGump : Gump
    {
        ...
        public CurrentRecipesGump(Mobile from)
            : this(from, 0, null)
        {
        }
        ...
        */
        ...
        public CurrentRecipesGump(Mobile from, int page, List<RecipeScrollDefinition> list)
            : base(12, 24)
        {
            ...
            if (list == null)
            {
                Shard.Debug("Criando receitas");
                list = new List<RecipeScrollDefinition>();
                var i = 0;
                foreach (var id in Recipe.Recipes.Keys)
                {
                    i++;
            ...
            AddHtml(266, 32, 200, 32, "Receitas", LabelColor, false, false); // Recipe Book

            AddHtml(147, 64, 200, 32, "Item", LabelColor, false, false); // Item
            //AddHtml(246, 64, 200, 32, "ID", LabelColor, false, false); // Expansion
            AddHtml(336, 64, 200, 32, "Crafting", LabelColor, false, false); // Crafting
            AddHtml(429, 64, 100, 32, "Aprendida", LabelColor, false, false); // Amount
            ...
            */
            ...
            for (int i = index; i < (index + count) && i >= 0 && i < list.Count; ++i)
            {
                ...
                {
                    if (Recipe.Recipes[recipe.RecipeID].TextDefinition.String != null)
                    {
                        AddHtml(103, y, 230, 32, Recipe.Recipes[recipe.RecipeID].TextDefinition.String, 0xFFFFFF, false, false); // ~1_val~
                    } else
                    {
                        AddHtmlLocalized(103, y, 230, 32, Recipe.Recipes[recipe.RecipeID].TextDefinition.Number, "#103221", 0xFFFFFF, false, false); // ~1_val~
                    }
                    //AddLabel(235, y, 0x480, ""+recipe.RecipeID);
                    AddHtml(316, y, 100, 20, recipe.Skill.ToString(), 0xFFFFFF, false, false); // ~1_val~
                    if (recipe.Amount >= 1)
                    {
                        AddLabel(421, y, 0x480, "Aprendida");
                    }
                }
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch (index)
            {
                ...
                case 1:
                    {
                        //from.SendGump(new RecipeScrollFilterGump(from));
                        break;
                    }
                case 2:
                    {
                        if (m_Page > 0)
                            from.SendGump(new CurrentRecipesGump(from, m_Page - 1, m_List));
