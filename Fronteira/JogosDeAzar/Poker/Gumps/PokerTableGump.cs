...
namespace Server.Engines.TexasHoldem
{
    public sealed class PokerTableGump : ListGump<PokerPlayer>
    {
        ...
        public override bool AutoRefresh { get { return false; } set { } }
            ...
            : base(user, null, 0, 0)
        {
            ...
            _Game.IsPlayer((PlayerMobile)User, out _Player);

            Closable = _Game.IsViewer((PlayerMobile)User) || _Game.IsIntermission();
                ...
                () =>
                {
                    ...
                    if (!IsIntermission) //Display if a player is a dealer or blind
                    {
                        ...
                        if (entry.Currency == 0 || entry.HasFolded)
                        {
                            AddHtml(x + 105, y + 34, 101, 22,
                                String.Format("<BIG>{0}</BIG>", entry.Currency == 0 ? "All-In" : "Correu")
                    ...
                    else
                    {
                        ...
                        AddHtml(x - 45, y + 34, 200, 22,
                            String.Format("{0}: {1:n0} moedas", lost ? "Perdeu" : "Ganhou", entry.AmountWon).WrapUOHtmlColor(lost ? Color.Red : Color.LawnGreen), false, false);
                    }
                    ...
                    if (indexDifference != 0 && (IsIntermission && !entry.HasFolded))
                    {
                        int lastY = y + 85;
