...
namespace Server.Engines.TexasHoldem
{
    public class PokerGame
    {
        ...
        public void ProcessTick()
        {
            ...
            else if (State > PokerGameState.Inactive && State < PokerGameState.Showdown)
            {
                ...
                if (IsBettingRound && CurrentTurn != null)
                {
                    ...
                    {
                        CurrentTurn.SendMessage("Seu turno acaba em " + CurrentTurn.TurnEnd.Seconds +
                                                " segundos. Se voce nao jogar voce vai dar Fold.");
                    }
            ...
            else if (IsIntermission())
            {
                ...
                {
                    PokerMessage(Dealer, "A proxima mao comeca em " + HandCoolDown.Seconds +
                                            " segundos.");
                }
        ...
        {
            PokerMessage(Dealer, "Jogadores insuficientes para continuar.");
            ...
            bool verbose = true)
        {
            ...
            switch (action)
            {
                case PlayerAction.Bet:
                    message = String.Format("I {0} {1}.", "apostar", bet);
                ...
                case PlayerAction.Raise:
                    message = String.Format("Vou {0} {1}.",
                        RoundActions.Exists(x => x == PlayerAction.Raise) ? "re-aumentar" : "aumentar", bet);
                ...
                case PlayerAction.AllIn:
                    ...
                    {
                        message = MinimumBet > player.Currency ? "Eu chamo: all-in." : "All in.";
        ...
        public void AssignNextTurn()
        {
            ...
            if (CurrentTurn != null)
            {
                ...
                CurrentTurn.Owner.SendGump(new PokerBetGump(CurrentTurn.Owner, this, CurrentTurn));
            }
        }
        ...
        {
            if (player.AccessLevel <= AccessLevel.VIP && Players.Any(p => p.Owner.NetState != null && player.NetState != null &&
            ...
            if (player.PokerJoinTimer > DateTime.UtcNow)
            {
                ...
                player.SendMessage("Voce nao pode entrar em outro jogo por " + nextuse.Seconds + " seconds.");
                return false;
            }

            if (player.Aggressed.Any(info => (DateTime.UtcNow - info.LastCombatTime) < TimeSpan.FromSeconds(60)))
            {
                player.SendMessage("Voce nao pode jogar em combate!");
                return false;
            }
            ...
            {
                player.SendMessage("Voce nao pode jogar estando em um grupo.");
                return false;
            }

            if (!Dealer.InRange(player.Location, 8))
            {
                player.PrivateOverheadMessage(MessageType.Regular, 0x22, true, "Muito longe",
                    player.NetState);
                return false;
            }
            ...
            if (IsPlayer(player))
            {
                player.SendMessage(0x22, "Voce ja esta jogando nessa mesa");
                return false;
            }

            if (Players.Count >= Dealer.MaxPlayers)
            {
                player.SendMessage(0x22, "Esta mesa esta cheia");
                return false;
            }

            if (!Dealer.Seats.Exists(x => !Dealer.SeatTaken(x)))
            {
                player.SendMessage(0x22, "Desculpe esta mesa esta cheia");
                return false;
            }

            if (!Banker.Withdraw(player, buyin))
            {
                player.SendMessage(0x22, "Voce nao tem dinheiro suficiente no banco para jogar.");
                return false;
            }
