...
namespace Server.Engines.ResortAndCasino
{
    ...
    public class ChucklesLuck : BaseDiceGame
    {
        ...
        public override void BeginRollDice()
        {
            ...
            Player.PrivateOverheadMessage(MessageType.Regular, 0x35, false, "Apostou " + CurrentBet.ToString() + " em " + BettingOn.ToString(), Player.NetState); // *bets ~1_AMT~ chips on ~2_PROP~*
        }
        ...
        public override void OnDiceRolled()
        {
            ...
            Dealer.PrivateOverheadMessage(MessageType.Regular, 0x35, false, "Rolaram os dados, caiu "+Roll[0] +"/"+ Roll[1] +"/"+ Roll[2], Player.NetState); // *rolls the dice; they land on ~1_FIRST~ ~2_SECOND~ ~3_THIRD~*

            if (matches == 0)
            {
                Dealer.PrivateOverheadMessage(MessageType.Regular, 0x35, false, "Perde-se "+String.Format("{0}\t{1}", Player.Name, CurrentBet.ToString(CultureInfo.GetCultureInfo("en-US"))), Player.NetState); // *rakes in ~1_NAME~'s ~2_VAL~-chip bet*
            }
            else
            {
                ...
                Winner = true;
                OnWin();
                Dealer.PrivateOverheadMessage(MessageType.Regular, 0x35, false,"Pagou "+ winnings.ToString(CultureInfo.GetCultureInfo("en-US") +" para "+ Player.Name), Player.NetState); // *pays out ~2_VAL~ chips to ~1_NAME~*
            }
        }
    ...
    public class HiMiddleLow : BaseDiceGame
    {
        ...
        public override void BeginRollDice()
        {
            ...
            Player.PrivateOverheadMessage(MessageType.Regular, 0x35, false, "Apostou "+ CurrentBet.ToString() +" em "+ ((HighMiddleLowType)BettingOn).ToString(), Player.NetState); // *bets ~1_AMT~ chips on ~2_PROP~*
        }
        ...
        public override void OnDiceRolled()
        {
            ...
            else
            {
                ...
                Dealer.PrivateOverheadMessage(MessageType.Regular, 0x35, false, String.Format("Os dados rolaram e saiu {0}\t{1}\t{2}", Roll[0], Roll[1], Roll[2]), Player.NetState); // *rolls the dice; they land on ~1_FIRST~ ~2_SECOND~ ~3_THIRD~*
                ...
                if (winnings > 0)
                {
                    ...
                    Dealer.PrivateOverheadMessage(MessageType.Regular, 0x35, false, String.Format("Pagou {1} para {0}", Player.Name, winnings.ToString(CultureInfo.GetCultureInfo("en-US"))), Player.NetState); // *pays out ~2_VAL~ chips to ~1_NAME~*
                }
                else
                {
                    Dealer.PrivateOverheadMessage(MessageType.Regular, 0x35, false, String.Format("Levou {1} de {0}", Player.Name, CurrentBet.ToString(CultureInfo.GetCultureInfo("en-US"))), Player.NetState); // *rakes in ~1_NAME~'s ~2_VAL~-chip bet*
                }
            }
        }
    ...
    public class DiceRider : BaseDiceGame
    {
        ...
        public override void BeginRollDice()
        {
            ...
            if(RollNumber == 1)
                Player.PrivateOverheadMessage(MessageType.Regular, 0x35, false, "Apostou "+(CurrentBet / 3).ToString(CultureInfo.GetCultureInfo("en-US")), Player.NetState); // *bets ~1_AMT~ chips on ~2_PROP~*
        }
        ...
        public override void OnDiceRolled()
        {
            ...
            {
                RollDice(3);
                Dealer.PrivateOverheadMessage(MessageType.Regular, 0x35, false, String.Format("Rolaram os dados e caiu {0}\t{1}\t{2}", GetRoll(0), GetRoll(1), GetRoll(2)), Player.NetState); // *rolls the dice; they land on ~1_FIRST~ ~2_SECOND~ ~3_THIRD~*
            }
            else
            {
            ...
            if (RollNumber == 3)
            {
                ...
                {
                    Dealer.PrivateOverheadMessage(MessageType.Regular, 0x35, 1153376, String.Format("Levou {1} de \t{0}", Player.Name, TotalBet.ToString(CultureInfo.GetCultureInfo("en-US"))), Player.NetState); // *rakes in ~1_NAME~'s ~2_VAL~-chip bet*
                }
            }
            else
            {
