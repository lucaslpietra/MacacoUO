...
namespace Server.Items
{
	public abstract class BaseWineGrapes : Item
	{
		...
		{
			if ( Amount > 1 ) list.Add( 1050039, "{0}\t{1}", Amount, " "+WinemakingResources.GetName( m_Variety )+" Uvas" );
			else list.Add( "Monte com "+WinemakingResources.GetName( m_Variety )+" uvas" );
		}

		public override void OnSingleClick( Mobile from )
		{
			if ( this.Amount > 1 ) this.LabelTo( from, "{0} Uvas : {1}", WinemakingResources.GetName( m_Variety ), Amount );
			else this.LabelTo( from, "{0} Uvas", WinemakingResources.GetName( m_Variety ) );
		}
