...
namespace Server.Items.Crops
{
	public class BaseGrapeVine : Item
	{
		...
		public override void AddNameProperty( ObjectPropertyList list )
 		{
 			list.Add( 1060658, "Variedade\t{0}", WinemakingResources.GetName( m_Variety ) );
		...
		{
			this.LabelTo( from, "Variedade : {0} ", WinemakingResources.GetName( m_Variety ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.Mounted )
			{
				from.SendMessage( "Nao pode pegar montado." );
				return;
			}
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(5) )
			{
				...
				if ( lumberValue < 0 )
				{
					from.SendMessage( "Voce nao faz ideia de como colher isso." );
					return;
				}
				...
				if ( from.InRange( this.GetWorldLocation(), 2 ) )
				{
					if( m_Placer == null || from == m_Placer || from.AccessLevel >= AccessLevel.GameMaster )
					{
						if ( m_yield < 1 )
						{
							from.SendMessage( "Nada aqui pega pegar." );
						}
						else
						{
							...
							if ( pick == 0 )
							{
								from.SendMessage( "Voce nao conseguiu pegar nada." );
								return;
							}

							m_yield -= pick;
							from.SendMessage( "Voce pegou {0} uva{1}!", pick, ( pick == 1 ? "" : "s" ) );
					...
					{
						from.SendMessage( "Isso nao eh seu danado(a)." );
					}
				}
				else
				{
			...
			{
				from.SendMessage( "Espere um pouco...." );
			}
		}
