...
namespace Server.Items
{
    public class VinyardGroundAddonDeed : BaseAddonDeed
    {
        ...
        private void BoundingBox_Callback(Mobile from, Map map, Point3D start, Point3D end, object state)
        {
            ...
            if (width < 2 || height < 2)
                from.SendMessage("Precisa de um 3x3 livre.");
...
namespace Server.Gumps
{
    public class VinyardGroundGump : Gump
    {
        ...
        public VinyardGroundGump(BaseAddonDeed deed, IPoint3D p, Map map, int width, int height) : base(30, 30)
        {
            ...
            AddHtml(13, 13, 379, 20, "<BASEFONT COLOR=WHITE>Escolha um tipo de solo</BASEFONT>", false, false);
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (info.ButtonID >= 1)
            {
                ...
                if (res == AddonFitResult.Valid)
                    ...
                    from.SendMessage("Feche todas as portas por favor.");
    ...
    public class VinyardGroundPlacedGump : Gump
    {
        ...
        public VinyardGroundPlacedGump(BaseAddonDeed deed) : base(30, 30)
        {
            ...
            AddHtml(13, 13, 379, 20, "<BASEFONT COLOR=WHITE>Vinicula Colocada Com Sucesso</BASEFONT>", false, false);

            AddAlphaRegion(12, 39, 426, 199);

            AddHtml(15, 50, 420, 185, "<BODY>" +
            "<BASEFONT COLOR = AMARELO> Seu complemento de terra da vinheta foi colocado com sucesso! <BR>" +
"<BASEFONT COLOR = AMARELO> Agora você pode começar a colocar videiras em sua vinha usando um" +
"<BASEFONT COLOR = AMARELO> ferramenta de posicionamento de videira. <BR> <BR>" +
"<BASEFONT COLOR = AMARELO> Você pode excluir este complemento de terra da vinheta a qualquer momento." +
"<BASEFONT COLOR = AMARELO> Para fazer isso ... <BR>" +
"<BASEFONT COLOR = AMARELO> 1. Exclua todas as videiras primeiro. <BR>" +
"<BASEFONT COLOR = AMARELO> 2. Clique duas vezes no complemento vinyard e aceite o prompt para excluir. <BR>" +
"<BASEFONT COLOR = AMARELO> * Nota * Você deve estar dentro de 3 blocos do canto oeste para excluir o complemento. <BR> <BR>" + "</BODY>", false, false);
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                case 0:
                    {
                        from.SendMessage("Curta sua vinicula :).");
                        break;
                    }
            }
        }
