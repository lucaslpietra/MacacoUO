...
namespace Server.Items
{
	public abstract class BaseCraftCoffee : Item, ICraftable
	{
        ...
        static public bool Thirsty(Mobile from, int fillFactor)
        {
            ...
            {
                from.Thirst = 20;
                from.SendMessage("Voce bebeu mas esta cheio!");
            }
            else
            {
                from.Thirst = iThirst;
                if (iThirst < 5) from.SendMessage("Voce bebeu mas continua com sede.");
                else if (iThirst < 10) from.SendMessage("Voce bebeu e esta um pouco melhor.");
                else if (iThirst < 15) from.SendMessage("Depois de beber voce se sente muito melhor.");
                else from.SendMessage("Voce ta com maior barrigao de liquido agora.");
            }
            return true;
        }
