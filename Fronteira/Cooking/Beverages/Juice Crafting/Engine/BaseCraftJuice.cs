	...
	public abstract class BaseCraftJuice : Item, ICraftable
	{
		...
		public override void AddNameProperties( ObjectPropertyList list )
 		{
			...
			if ( m_Quality == JuiceQuality.Exceptional ) list.Add( 1060847, "Premium\t{0} Suco", JuiceType );
 			else list.Add( 1060847, "\t{0} Suco", JuiceType );
 		}
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if (this.Name == null )
			{
				...
				else this.LabelTo( from, "Garrafa de suco" );
			}
			else this.LabelTo( from, "{0} Farms", this.Name );
			JuiceType = JuicingResources.GetName( m_Variety );
			if ( m_Quality == JuiceQuality.Exceptional ) this.LabelTo( from, "Premium {0} Suco", JuiceType );
 			else this.LabelTo( from, "{0} Suco", JuiceType );
		}
