...
{
	public class HerdingBaseCrop : Item, IChopable
	{
		...
		public HerdingBaseCrop( int itemID ) : base( itemID )
        {
        }
		...
		public HerdingBaseCrop( Serial serial ) : base( serial )
        {
        }
	...
	{
		public static bool CanWorkMounted{ get{ return true; } }
		...
		public static bool CheckCanGrow( HerdingBaseCrop crop, Map map, int x, int y )
		{
		...
		public class GrowTimer : Timer
		{
			...
			public GrowTimer( Item seedling, Type croptype, Mobile sower ) : base( TimeSpan.FromSeconds( 600 ), TimeSpan.FromSeconds( 12 ) )
			{
				...
				cookValue = sower.Skills[SkillName.Herding].Value / 50;
			}
        ...
        public static ArrayList CheckCrop(Point3D pnt, Map map, int range)
		{
            ...
            { 
                if ( ( crop != null ) && ( crop is HerdingBaseCrop ) ) crops.Add( (HerdingBaseCrop)crop );
            }
			eable.Free();
			return crops;
		}

		public class Weeds : HerdingBaseCrop
		{
			...
			public override void OnDoubleClick(Mobile from)
			{
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
                    if ((from == m_sower) || (DateTime.UtcNow >= planted.AddDays(3)))
					{
						...
						from.SendMessage("Voce pegou a erva.");
						DandelionSeed fruit = new DandelionSeed();
						from.AddToBackpack( fruit );
						this.Delete();
					}
					else from.SendMessage("Ainda esta ruim de puxar.");
				}
				else
				{
