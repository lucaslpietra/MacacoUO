...
{
	public class WheatCrop : HerdingBaseCrop
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( from != m_sower )
            { 
                from.SendMessage( "Esta planta nao e sua !!!" ); 
                return; 
            }
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				if ( cookValue == 0 )
                {
                    from.SendMessage( "Voce nao tem skill suficiente para colher esta planta." );
                    return; 
                }
				if ( from.InRange( this.GetWorldLocation(), 1 ) )
				{
					...
					else
					{
                        ...
                        if ( pick == 0 ) 
                        { 
                            from.SendMessage( "Voce nao conseguiu colher nada." ); return; 
                        }
