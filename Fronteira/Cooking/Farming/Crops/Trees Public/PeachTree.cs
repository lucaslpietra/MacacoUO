...
{
	public class PeachSapling : HerdingBaseCrop
	{
	...
	public class PeachTree : BaseTree
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( DateTime.UtcNow > lastpicked.AddSeconds(3) )
			{
				...
				int cookValue = (int)from.Skills[SkillName.Herding].Value / 30;
				if ( from.Mounted )
					++cookValue;

				if ( cookValue < 0 )
				{
					from.SendMessage( "Voce nao faz ideia de como colher isto." );
					return;
				}
				...
				if ( from.InRange( this.GetWorldLocation(), 2 ) )
				{
					if ( m_yield < 1 )
					{
						from.SendMessage( "Nao tem nada aqui para ser colhido." );
					}
					else
					{
						...
						if ( pick == 0 )
						{
							from.SendMessage( "Voce nao conseguiu pegar nenhuma fruta." );
							return;
						}

						m_yield -= pick;
						from.SendMessage( "Voce colheu {0} peach{1}!", pick, ( pick == 1 ? "" : "es" ) );
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 2 ) )
			{
				...
				{
					if ( ( TreeHelper.TreeOrdinance ) && ( from.AccessLevel <= AccessLevel.VIP ) )
					{
		...
		private class StumpTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				i_tree.Delete();
				m_chopper.SendMessage( "Voce colocou madeiras e frutas em sua mochila" );
			}
		}
