...
{
 	public abstract class CookingBaseCrop : Item
 	{
 		public int version;
 		...
 		public CookingBaseCrop( int ItemID, int MinDelay, int MaxDelay ) : base ( ItemID )
 		{
 			MinAgeTime = MinDelay;
 			MaxAgeTime = MaxDelay;
 			Movable = false;
 		...
 		public CookingBaseCrop( Serial serial ) : base( serial )
 		{
 		}
		...
		public virtual void Start( CookingBaseCrop item, TimeSpan AgeDelay )
 		{
 			m_AgeDelay = AgeDelay;
 		...
 		public override void OnDoubleClick( Mobile from )
 		{
 			...
 			if ( FullGrown == true )
 			{
 				...
 				if ( DeleteWhenDone )
				{
 				...
 				from.SendMessage( "Isto esta ainda maduro para colher." );
 		}
 		
 		private class CropTimer : Timer
 		{
 			private CookingBaseCrop m_Crop;
 			
 			public CropTimer( CookingBaseCrop item, TimeSpan Delay ): base ( Delay )
 			{
 				m_Crop = item;
 		...
 		public virtual bool CheckRange( Mobile from, int Range, CookingBaseCrop m_Crop, Point3D loc )
 		{
 			bool inRange = ( from.Map == m_Crop.Map && from.InRange( loc, Range ) );
 			
 			if ( !inRange ) from.SendMessage( "Muito longe" ); // that is too far away.
