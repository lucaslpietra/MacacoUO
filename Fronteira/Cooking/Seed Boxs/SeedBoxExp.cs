...
//			return;
            ...
            if ( Movable )
            {
                from.SendMessage( "O item nao esta trancado !" );
                return ;
            }
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			list.Add( 1060662,"{0}\t{1}" ,"Sementes", SeedCount() );
		}
        ...
        public override bool OnDragDrop( Mobile from, Item dropped )
        {
            ...
            if ( !( dropped is Seed ) )
            {
                from.SendMessage( "Voce so pode colocar sementes aqui." );
                return false;
            }

			if( SeedCount() > m_maxSeeds )
			{
				from.SendMessage( "Nao cabe mais sementes." );
				return false;
			}
			...
			if( !seed.ShowType )
			{
				from.SendMessage( "Voce nem conhece essa semente direito, nao pode colocar aqui." );
				return false;
			}
