			...
			if (o is Item && (((Item)o).IsChildOf(from.Backpack) || ((Item)o).IsChildOf(from.BankBox)))
            {
				...
				if (curItem is CottonSeed)
				{
					if (CottonSeed + curItem.Amount > StorageLimit)
						from.SendMessage("Tentando adicionar "+ ( (CottonSeed + curItem.Amount) - m_StorageLimit ) +" demais! Apenas cabe "+ m_StorageLimit +" deste recurso.");
					else
					{
				...
				else if (curItem is FlaxSeed)
				{
					if (FlaxSeed + curItem.Amount > StorageLimit)
						from.SendMessage("Adicionando "+ ( (FlaxSeed + curItem.Amount) - m_StorageLimit ) +" demais! So cabe "+ m_StorageLimit +" disso.");
					else
					{
				...
				else if (curItem is HaySeed)
				{
					if (HaySeed + curItem.Amount > StorageLimit)
						from.SendMessage("Adicionando "+ ( (HaySeed + curItem.Amount) - m_StorageLimit ) +" demais! So cabe "+ m_StorageLimit +" disso.");
					else
					{
				...
				else if (curItem is OatsSeed)
				{
					if (OatsSeed + curItem.Amount > StorageLimit)
                        from.SendMessage("Adicionando " + ((OatsSeed + curItem.Amount) - m_StorageLimit) + " demais! So cabe " + m_StorageLimit + " disso.");
                    else
					{
				...
				else if (curItem is RiceSeed)
				{
					if (RiceSeed + curItem.Amount > StorageLimit)
                        from.SendMessage("Adicionando " + ((RiceSeed + curItem.Amount) - m_StorageLimit) + " demais! So cabe " + m_StorageLimit + " disso.");
                    else
					{
				...
				else if (curItem is SugarcaneSeed)
				{
					if (SugarcaneSeed + curItem.Amount > StorageLimit)
                        from.SendMessage("Adicionando " + ((SugarcaneSeed + curItem.Amount) - m_StorageLimit) + " demais! So cabe " + m_StorageLimit + " disso.");
                    else
					{
				...
				else if (curItem is WheatSeed)
				{
					if (WheatSeed + curItem.Amount > StorageLimit)
                        from.SendMessage("Adicionando " + ((WheatSeed + curItem.Amount) - m_StorageLimit) + " demais! So cabe " + m_StorageLimit + " disso.");
                    else
					{
				...
				else if (curItem is BitterHopsSeed)
                {
                    if (BitterHopsSeed + curItem.Amount > StorageLimit)
                        from.SendMessage("Adicionando " + ((BitterHopsSeed + curItem.Amount) - m_StorageLimit) + " demais! So cabe " + m_StorageLimit + " disso.");
                    else
                    {
                ...
                else if (curItem is ElvenHopsSeed)
                {
                    if (ElvenHopsSeed + curItem.Amount > StorageLimit)
                        from.SendMessage("Adicionando " + ((ElvenHopsSeed + curItem.Amount) - m_StorageLimit) + " demais! So cabe " + m_StorageLimit + " disso.");
                    else
                    {
                ...
                else if (curItem is SnowHopsSeed)
                {
                    if (SnowHopsSeed + curItem.Amount > StorageLimit)
                        from.SendMessage("Adicionando " + ((SnowHopsSeed + curItem.Amount) - m_StorageLimit) + " demais! So cabe " + m_StorageLimit + " disso.");
                    else
                    {
                ...
                else if (curItem is SweetHopsSeed)
                {
                    if (SweetHopsSeed + curItem.Amount > StorageLimit)
                        from.SendMessage("Adicionando " + ((SweetHopsSeed + curItem.Amount) - m_StorageLimit) + " demais! So cabe " + m_StorageLimit + " disso.");
                    else
                    {
    ...
    public class FarmersSeedBoxGump : Gump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
			...
			else if (info.ButtonID == 1)
			{
				...
				{
					m_From.SendMessage("Voce nao tem isso!");
                    m_From.SendGump(new FarmersSeedBoxGump(m_From, m_Key));
