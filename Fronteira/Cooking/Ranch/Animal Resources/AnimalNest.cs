...
namespace Server.Items
{
	public class AnimalNest : Item
	{
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if (m_HasEggs == 1) list.Add(m_HasEggs + " ovo");
			else if (m_HasEggs > 1) list.Add(m_HasEggs + " ovos");
		}
			...
			if(from.InRange( this.GetWorldLocation(), 1 ))
			{
				if (m_Owner != null)
				{
					if (m_HasEggs > 0)
					{
						...
						from.SendMessage("Voce pegou ovos do ninho.");
						if (m_HasEggs < 1) ItemID = 6869;
					}
					else 
					{
						from.SendMessage("O ninho esta vaziu.");
						ItemID = 6869;
					}
				}
				else 
				{
					Movable = true;
					m_Owner = from;
					from.SendMessage("Voce esvaziou o ninho.");
				}
			}
			else from.SendMessage("Muito longe!");
		}
